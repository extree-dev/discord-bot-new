...
// голый счёт очков.
//
// По прямому запросу администратора прокачка замедлена — раньше
// POINTS_PER_LEVEL=300 давало Хранителя (100 уровень) за 30 000 очков,
// администратор счёл это слишком быстрым и явно задал новый ориентир:
// 350 000 очков на Хранителя. 350000 / 100 = 3500 — прямая замена
// константы, все пороги ярусов и вся остальная арифметика (getLevelNumber/
// getLevelIndex/getLevel) от неё производные и пересчитываются сами.
const POINTS_PER_LEVEL = 3500;
