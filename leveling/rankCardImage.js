...
async function renderRankCard({
    ...
    // Две строки статистики под полосой, одна над другой (а не одна
    // строка слева+справа): сколько уровней осталось до следующего
    // яруса/роли (на максимальном ярусе — что расти по ролям уже некуда,
    // хотя сам уровень продолжает расти), и из чего набран счёт
    // (сообщения + голос + сам счёт) — активность, а не только итоговое
    // число. Раньше обе строки делили одну Y-координату (слева/справа) —
    // при достаточно длинном тексте с любой стороны (длинный титул яруса,
    // много сообщений, счёт на 6 знаков после увеличения
    // POINTS_PER_LEVEL) они наезжали друг на друга посередине карточки.
    ...
    const remainingLabel = level.next
        ...
        : 'Максимальный ярус';
    ctx.fillText(remainingLabel, TEXT_X, 196);
    ctx.fillText(
