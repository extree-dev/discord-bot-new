...
// Право на решение — то же ModerateMembers, что и у modqueue/большинства
...
const handleApproveButton = async interaction => {
    if (!canModerate(interaction.member)) {
        await interaction.reply({
            embeds: [errorEmbed('Одобрять предложения может только модерация.')],
    ...
    // channel-gone — заявка была валидной, но исходный канал с тех пор
    // удалили: считаем решённой (публиковать некуда), просто сообщаем об
    // этом в самой карточке вместо "Одобрено".
...
const handleRejectButton = async interaction => {
    if (!canModerate(interaction.member)) {
        await interaction.reply({
            embeds: [errorEmbed('Отклонять предложения может только модерация.')],
            flags: MessageFlags.Ephemeral,
        });
        return;
    }
