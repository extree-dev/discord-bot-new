...
- `/rules` и `scripts/setup-rules.js` — правила сервера, оформленные только официальной Markdown-разметкой Discord (`#`, `##`, `###`, списки через `- `), без эмодзи; скрипт публикует и закрепляет их в канале `#правила`, повторный запуск редактирует то же сообщение вместо дублирования.
